...
public class AntifraudService : IAntifraudService
{
    ...
    public async Task<bool> SendPurchaseAsync(CompleteEventWebModelRequest request)
    {
        ...
        var notificationResponse = await SendNotificationToAntifraud(new ProtocolRequest()
        {
            RequestUrl = _servicesConfig.AntifraudUrl,
    ...
    private async Task<ProtocolResponse<string>> SendNotificationToAntifraud(ProtocolRequest request)
    {
        return await _bambooSerilogContext.InvokeAndLogDataProvider(
            LogHelper.GetDataProviderConnectionLog(nameof(SendNotificationToAntifraud), request.RequestUrl, string.Empty), async () =>
            {
